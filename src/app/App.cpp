...
namespace {
    ...
    constexpr const char* kTypographyPreviewWords[] = {
        "minimum", "encyclopaedia", "state-of-the-art", "HTTP/2", "well-known",
        "rhythms", "illumination",  "WAVEFORM",         "I",
    };
    constexpr size_t kTypographyPreviewWordCount = sizeof(kTypographyPreviewWords) / sizeof(kTypographyPreviewWords[0]);
    ...
    size_t packedLifeWordCount(size_t cellCount) {
        return (cellCount + 31U) / 32U;
    }
    ...
    void setPackedLifeCellAt(std::vector<uint32_t>& cells, uint16_t columns, uint16_t rows, int x, int y, bool alive) {
    ...
    void clearPackedLifeRect(std::vector<uint32_t>& cells, uint16_t columns, uint16_t rows, int x, int y, int width,
                             int height) {
    ...
    void stampPackedLifePattern(std::vector<uint32_t>& cells, uint16_t columns, uint16_t rows, const LifePoint* points,
                                size_t pointCount, int originX, int originY) {
    ...
    void clearAndStampPackedLifePattern(std::vector<uint32_t>& cells, uint16_t columns, uint16_t rows,
                                        const LifePoint* points, size_t pointCount, int originX, int originY, int width,
                                        int height) {
    ...
    bool readLogicalPrimaryButtonHeld() {
        return Input::hasControl(Board::Input::currentControls(), Input::InputPrimary);
    }
...
void App::begin() {
    ...
    uiLanguage_ =
        Localization::sanitizeLanguage(preferences_.getUChar(kPrefUiLanguage, static_cast<uint8_t>(uiLanguage_)));
    readerMode_ = readerModeFromSetting(preferences_.getUChar(kPrefReaderMode, static_cast<uint8_t>(readerMode_)));
    handednessMode_ =
        handednessModeFromSetting(preferences_.getUChar(kPrefHandedness, static_cast<uint8_t>(handednessMode_)));
    ...
    switch (preferences_.getUChar(kPrefBatteryLabelMode, static_cast<uint8_t>(batteryLabelMode_))) {
    ...
    typographyConfig_.typeface =
        readerTypefaceFromSetting(preferences_.getUChar(kPrefReaderTypeface,
                                                        static_cast<uint8_t>(typographyConfig_.typeface)));
    ...
    typographyConfig_.trackingPx =
        static_cast<int8_t>(clampIntSetting(preferences_.getChar(kPrefTypographyTracking, typographyConfig_.trackingPx),
                                            kTypographyTrackingMin, kTypographyTrackingMax));
    typographyConfig_.anchorPercent =
        static_cast<uint8_t>(clampIntSetting(preferences_.getUChar(kPrefTypographyAnchor,
                                                                   typographyConfig_.anchorPercent),
                                             kTypographyAnchorMin, kTypographyAnchorMax));
    typographyConfig_.guideHalfWidth =
        static_cast<uint8_t>(clampIntSetting(preferences_.getUChar(kPrefTypographyGuideWidth,
                                                                   typographyConfig_.guideHalfWidth),
...
void App::update(uint32_t nowMs) {
    ...
    while (inputInitialized_ && Input::poll(inputEvent, nowMs)) {
        lastActivityMs_ = nowMs;
        handleInputEvent(inputEvent, nowMs);
        if (powerOffStarted_) {
            return;
        }
    }
...
const char* App::inputGestureName(Input::Gesture gesture) const {
    switch (gesture) {
    case Input::Gesture::ShortPressed:
        return "ShortPressed";
    case Input::Gesture::LongPressed:
        return "LongPressed";
    case Input::Gesture::TriplePressed:
        return "TriplePressed";
    case Input::Gesture::TouchStart:
        return "TouchStart";
    case Input::Gesture::TouchMove:
        return "TouchMove";
    case Input::Gesture::TouchEnd:
        return "TouchEnd";
    case Input::Gesture::Tapped:
        return "Tapped";
    case Input::Gesture::TopEdgeSwiped:
        return "TopEdgeSwiped";
    case Input::Gesture::BottomEdgeSwiped:
        return "BottomEdgeSwiped";
    case Input::Gesture::None:
    default:
        return "None";
    }
}
...
bool App::handleInputEvent(const Input::Event& event, uint32_t nowMs) {
    if (event.controls == Input::InputNone || event.gesture == Input::Gesture::None) {
        return false;
    }

    if (Input::isTouchEvent(event)) {
        Serial.printf("[input] gesture=%s x=%u y=%u state=%s\n", inputGestureName(event.gesture), event.x, event.y,
                      stateName(state_));
    }

    if (state_ == AppState::Standby) {
        return handleStandbyInput(event, nowMs);
    }
    if (state_ == AppState::Menu) {
        return handleMenuInput(event, nowMs);
    }
    return handleReaderInput(event, nowMs);
}

bool App::handleStandbyInput(const Input::Event& event, uint32_t nowMs) {
    if (event.gesture == Input::Gesture::ShortPressed || event.gesture == Input::Gesture::LongPressed
        || event.gesture == Input::Gesture::Tapped || event.gesture == Input::Gesture::TouchStart) {
        exitStandby(nowMs);
        return true;
    }
    return false;
...
bool App::handleMenuInput(const Input::Event& event, uint32_t nowMs) {
    ...
    if (Input::hasControl(event.controls, Input::InputPrimary) && event.gesture == Input::Gesture::TriplePressed) {
        Serial.println("[input] primary triple press -> standby");
        enterStandby(nowMs);
        return true;
    }
    ...
    if (Input::hasControl(event.controls, Input::InputPower) && event.gesture == Input::Gesture::ShortPressed) {
        toggleMenuFromPowerButton(nowMs);
        return true;
    }
...
bool App::handleReaderInput(const Input::Event& event, uint32_t nowMs) {
    if (Input::isTouchEvent(event)) {
        ...
        applyPausedTouchGesture(event, nowMs);
        return true;
    }

    if (state_ == AppState::CompanionSync && Input::hasControl(event.controls, Input::InputPrimary)
    ...
    if (Input::hasControl(event.controls, Input::InputPower) && event.gesture == Input::Gesture::LongPressed) {
        if (Board::Power::supportsSoftwarePowerOff()) {
            enterPowerOff(nowMs);
        } else {
            enterStandby(nowMs);
        }
...
void App::applyTypographySettings(uint32_t nowMs, bool rerender) {
    ...
    Serial.printf("[typography] face=%s highlight=%s track=%d anchor=%u guideWidth=%u guideGap=%u\n",
                  readerTypefaceLabel().c_str(), focusHighlightLabel().c_str(),
                  static_cast<int>(typographyConfig_.trackingPx), static_cast<unsigned int>(effectiveAnchorPercent()),
...
void App::cycleBrightness() {
    ...
    Serial.printf("[display] brightness level %u/%u (%u%%)\n", static_cast<unsigned int>(brightnessLevelIndex_ + 1),
                  static_cast<unsigned int>(kBrightnessLevelCount), static_cast<unsigned int>(percent));
...
bool App::updateBatteryStatus(uint32_t nowMs, bool force) {
    if (!force) {
        const bool lowBatteryKnown = batteryPresent_ && batterySampleInitialized_
                                  && (batteryFilteredVoltage_ <= kBatteryLowWarningVoltage
                                      || batteryDisplayedPercent_ <= kBatteryLowWarningPercent);
        uint32_t sampleIntervalMs = lowBatteryKnown ? kBatteryLowSampleIntervalMs : kBatterySampleIntervalMs;
    ...
    if (Board::Power::readBatteryStatus(status)) {
        ...
        if (!batterySampleInitialized_) {
            ...
            const int filteredPercent = std::max(0, std::min(100, static_cast<int>(batteryFilteredPercent_ + 0.5f)));
            const int delta = filteredPercent - static_cast<int>(batteryDisplayedPercent_);
            if (force || abs(delta) >= kBatteryDisplayHysteresisPercent || filteredPercent <= 10
                || filteredPercent >= 99) {
            ...
            if (batteryDisplayedPercent_ > batteryRuntimeAnchorPercent_) {
                ...
                if (percentDrop >= kBatteryRuntimeMinDropPercent && elapsedMs >= kBatteryRuntimeMinElapsedMs) {
    ...
    if (!batteryLabel_.isEmpty()) {
        Serial.printf("[power] battery %.2f V raw=%u%% shown=%u%% label=%s\n", status.voltage,
                      static_cast<unsigned int>(status.percent), static_cast<unsigned int>(batteryDisplayedPercent_),
                      batteryLabel_.c_str());
    } else {
...
void App::handleBatteryProtection(uint32_t nowMs) {
    ...
    const bool critical =
        batteryFilteredVoltage_ <= kBatteryCriticalVoltage || batteryDisplayedPercent_ <= kBatteryCriticalPercent;
    if (critical) {
    ...
    const bool low =
        batteryFilteredVoltage_ <= kBatteryLowWarningVoltage || batteryDisplayedPercent_ <= kBatteryLowWarningPercent;
    if (!low) {
        return;
    }

    if (lastLowBatteryWarningMs_ == 0 || nowMs - lastLowBatteryWarningMs_ >= kBatteryLowWarningRepeatMs) {
...
String App::onOffLabel(bool enabled) const {
    return enabled ? uiText(UiText::On) : uiText(UiText::Off);
}
...
void App::handleReaderTap(uint16_t x, uint16_t y, uint32_t nowMs) {
    ...
    const bool sameRegion =
        recentTap
        && abs(static_cast<int>(x) - static_cast<int>(lastReaderTapX_)) <= static_cast<int>(kReaderDoubleTapSlopPx)
        && abs(static_cast<int>(y) - static_cast<int>(lastReaderTapY_)) <= static_cast<int>(kReaderDoubleTapSlopPx);
...
bool App::handleTopEdgeMenuSwipe(const TouchEvent& event, uint32_t nowMs, int deltaX, int deltaY, bool ended) {
    if (!Board::Config::ENABLE_TOP_EDGE_MENU_SWIPE || !ended || state_ == AppState::Menu || state_ == AppState::Standby
    ...
    const bool verticalDownSwipe =
        deltaY >= static_cast<int>(kMenuSwipeTriggerPx) && absDeltaY > absDeltaX + static_cast<int>(kAxisBiasPx);
...
bool App::handleBottomEdgeQuickSettingsSwipe(const TouchEvent& event, uint32_t nowMs, int deltaX, int deltaY,
                                             bool ended) {
    if (!Board::Config::ENABLE_BOTTOM_EDGE_QUICK_SETTINGS_SWIPE || !ended || state_ == AppState::Menu
        || state_ == AppState::Standby || state_ == AppState::Sleeping) {
        return false;
    }
    ...
    const bool startsNearBottom =
        pausedTouch_.startY >= static_cast<uint16_t>(Board::Config::DISPLAY_HEIGHT - kQuickSettingsSwipeBottomZonePx);
...
void App::applyMenuTouchGesture(const TouchEvent& event, uint32_t nowMs) {
    ...
    if (!Input::isTouchReleaseGesture(event.gesture)) {
        return;
    }
...
void App::applyFocusTimerTouch(const TouchEvent& event, uint32_t nowMs) {
    ...
    if (focusTimer_.isActiveTimerRunning() && !focusTimerCancelHoldTriggered_
        && event.gesture != Input::Gesture::TouchEnd && absDeltaX <= static_cast<int>(kFocusTimerCancelHoldMaxDriftPx)
...
void App::openFocusTimer() {
    ...
    focusTimerCancelHoldTriggered_ = false;
    menuScreen_ = (focusTimer_.state() == FocusTimer::State::GenreSelect) ? MenuScreen::FocusTimerGenres
...
void App::moveMenuSelection(int direction) {
    ...
    size_t itemCount = Board::Config::ENABLE_RESTRUCTURED_MENU ? static_cast<size_t>(RestructuredMenuItemCount)
    ...
    if (isSettingsMenuScreen(menuScreen_)) {
        ...
        const String selectedLabel = quickSyncSelectedIndex_ == QuickSyncWifi ? "Wi-Fi Sync" : "USB Sync";
...
void App::openSettings() {
    settingsSelectedIndex_ =
        Board::Config::ENABLE_RESTRUCTURED_MENU ? kSettingsHomeRestructuredDisplayIndex : kSettingsHomeDisplayIndex;
    menuScreen_ = MenuScreen::SettingsHome;
    rebuildSettingsMenuItems();
    renderSettings();
}
...
void App::selectSettingsItem(uint32_t nowMs) {
    ...
    case kSettingsPacingLongWordsIndex:
        pacingLongWordDelayMs_ = static_cast<uint16_t>(nextCyclicSetting(pacingLongWordDelayMs_, kPacingDelayMinMs,
                                                                         kPacingDelayMaxMs, kPacingDelayStepMs));
    ...
    case kSettingsPacingComplexityIndex:
        pacingComplexWordDelayMs_ =
            static_cast<uint16_t>(nextCyclicSetting(pacingComplexWordDelayMs_, kPacingDelayMinMs, kPacingDelayMaxMs,
                                                    kPacingDelayStepMs));
    ...
    case kSettingsPacingPunctuationIndex:
        pacingPunctuationDelayMs_ =
            static_cast<uint16_t>(nextCyclicSetting(pacingPunctuationDelayMs_, kPacingDelayMinMs, kPacingDelayMaxMs,
                                                    kPacingDelayStepMs));
...
void App::openWifiSettings() {
    if (Board::Config::ENABLE_RESTRUCTURED_MENU) {
        settingsSelectedIndex_ = configuredWifiSsid().isEmpty() ? kWifiSettingsRestructuredNetworkIndex
                                                                : kWifiSettingsRestructuredAutoUpdateIndex;
    } else {
        settingsSelectedIndex_ =
            configuredWifiSsid().isEmpty() ? kWifiSettingsChooseIndex : kWifiSettingsAutoUpdateIndex;
    }
...
void App::scanWifiNetworks() {
    ...
    for (const WifiNetworkInfo& network: wifiNetworks_) {
        wifiNetworkMenuItems_.push_back({network.ssid,
                                         wifiSecurityLabel(network.authMode) + "  " + String(network.rssi) + " dBm"});
    }

    wifiNetworkSelectedIndex_ = wifiNetworkMenuItems_.size() > 1 ? kWifiNetworksFirstItemIndex : kWifiNetworksBackIndex;
...
void App::selectWifiNetworkItem(uint32_t nowMs) {
    ...
    if (wifiNetworkRequiresPassword(network.authMode)) {
        ...
        openTextEntry(TextEntryPurpose::WifiPassword, network.ssid, "Password", "", initialValue, network.ssid, true,
                      kWifiPasswordMaxLength, MenuScreen::WifiNetworks);
        return;
    }
...
void App::openTextEntry(TextEntryPurpose purpose, const String& title, const String& prompt, const String& helperText,
                        const String& initialValue, const String& contextValue, bool masked, size_t maxLength,
...
void App::rebuildTextEntryButtons() {
    ...
    for (size_t rowIndex = 0; rowIndex < 3; ++rowIndex) {
        ...
        const int availableWidth =
            Board::Config::DISPLAY_WIDTH - (2 * kKeyboardMarginX) - static_cast<int>((keyCount - 1) * kKeyboardRowGap);
    ...
    const ControlButtonDef controls[] = {
        {"abc", TextEntryAction::SetLower, 11, false, textEntrySession_.mode == KeyboardMode::Lower},
        {"ABC", TextEntryAction::SetUpper, 11, false, textEntrySession_.mode == KeyboardMode::Upper},
        {"123", TextEntryAction::SetSymbols, 11, false, textEntrySession_.mode == KeyboardMode::Symbols},
        ...
        {textEntrySession_.masked ? (revealActive ? "hide" : "show") : "clear",
         textEntrySession_.masked ? TextEntryAction::ToggleMask : TextEntryAction::Clear, 13, false, revealActive},
...
void App::renderTextEntry() {
    ...
    const String visibleValue = (textEntrySession_.masked && !textEntrySession_.revealValue)
...
void App::selectTypographyTuningItem(uint32_t nowMs) {
    ...
    case TypographyTuningTracking:
        typographyConfig_.trackingPx =
            static_cast<int8_t>(nextCyclicSetting(typographyConfig_.trackingPx, kTypographyTrackingMin,
    ...
    case TypographyTuningAnchor: {
        const uint8_t anchorMin = (handednessMode_ == HandednessMode::Left) ? kLeftHandAnchorMin : kTypographyAnchorMin;
        const uint8_t anchorMax = (handednessMode_ == HandednessMode::Left) ? kLeftHandAnchorMax : kTypographyAnchorMax;
        const uint8_t nextAnchorPercent =
            static_cast<uint8_t>(nextCyclicSetting(effectiveAnchorPercent(), anchorMin, anchorMax));
    ...
    case TypographyTuningGuideWidth:
        typographyConfig_.guideHalfWidth =
            static_cast<uint8_t>(nextCyclicSetting(typographyConfig_.guideHalfWidth, kTypographyGuideWidthMin,
    ...
    case TypographyTuningGuideGap:
        typographyConfig_.guideGap =
            static_cast<uint8_t>(nextCyclicSetting(typographyConfig_.guideGap, kTypographyGuideGapMin,
                                                   kTypographyGuideGapMax));
...
bool App::startBackgroundOtaCheck(const OtaUpdater::Config& config) {
    ...
    BaseType_t created =
        xTaskCreatePinnedToCore(otaCheckTask, "ota_check", kOtaCheckTaskStackBytes, params, 1, nullptr, 0);
    if (created != pdPASS) {
...
void App::otaCheckTask(void* params) {
    ...
    copyOtaLabel(queuedResult.currentVersion, sizeof(queuedResult.currentVersion), result.currentVersion);
    copyOtaLabel(queuedResult.latestVersion, sizeof(queuedResult.latestVersion), result.latestVersion);
...
void App::pollOtaCheckResult(uint32_t nowMs) {
    ...
    while (xQueueReceive(otaCheckQueue_, &result, 0) == pdTRUE) {
        ...
        Serial.printf("[ota] background result code=%u current=%s latest=%s summary=%s detail=%s\n",
                      static_cast<unsigned int>(result.code), result.currentVersion, result.latestVersion,
...
void App::runFirmwareUpdate(const OtaUpdater::Config& config, bool automatic, uint32_t nowMs) {
    ...
    saveReadingPosition(true);
    const OtaUpdater::Result result = otaUpdater_.checkAndInstall(config, &App::handleStorageStatus, this);
...
void App::runRssFeedCheck(uint32_t nowMs) {
    ...
    Serial.printf("[rss] feeds=%u saved=%u skipped=%u summary=%s detail=%s\n",
                  static_cast<unsigned int>(result.feedsChecked), static_cast<unsigned int>(result.articlesSaved),
                  static_cast<unsigned int>(result.articlesSkipped), result.summary.c_str(), result.detail.c_str());
...
String App::uiText(UiText key) const {
    return Localization::text(uiLanguage_, key);
}
...
String App::uiLanguageLabel() const {
...
void App::openBookPicker(bool articlesOnly) {
    ...
    std::stable_sort(sortedBookIndices.begin(), sortedBookIndices.end(), [this](size_t leftIndex, size_t rightIndex) {
        const bool leftCurrent = usingStorageBook_ && leftIndex == currentBookIndex_;
        const bool rightCurrent = usingStorageBook_ && rightIndex == currentBookIndex_;
        if (leftCurrent != rightCurrent) {
            return leftCurrent;
        }

        const uint32_t leftRecent = bookRecentSequence(storage_.bookPath(leftIndex));
        const uint32_t rightRecent = bookRecentSequence(storage_.bookPath(rightIndex));
    ...
    if (!usbTransfer_.begin(true)) {
        ...
        if (storageReady_ && usingStorageBook_ && !currentBookPath_.isEmpty()) {
            ...
            if (refreshedBookIndex >= 0
                && loadBookAtIndex(static_cast<size_t>(refreshedBookIndex), nowMs, reloadOptions)) {
...
void App::exitUsbTransfer(uint32_t nowMs) {
    ...
    if (storageReady_) {
        ...
        if (refreshedBookIndex >= 0) {
            ...
            reloadOptions.allowEpubConversion = false;
            reloadOptions.rebuildTimeEstimate = false;
            if (loadBookAtIndex(static_cast<size_t>(refreshedBookIndex), nowMs, reloadOptions)) {
                reader_.seekTo(resumeIndex);
            } else {
...
void App::exitStandby(uint32_t nowMs) {
    ...
    if (nextState == AppState::Booting || nextState == AppState::Playing || nextState == AppState::CompanionSync
        || nextState == AppState::UsbTransfer || nextState == AppState::Standby || nextState == AppState::Sleeping) {
...
void App::seedStandbyLife(uint32_t nowMs) {
    const size_t cellCount = static_cast<size_t>(kStandbyLifeColumns) * static_cast<size_t>(kStandbyLifeRows);
    ...
    standbyScreensaverRng_ = nowMs ^ micros() ^ (static_cast<uint32_t>(reader_.currentIndex() + 1) * 2654435761UL)
    ...
    clearAndStampPackedLifePattern(standbyLifeCells_, kStandbyLifeColumns, kStandbyLifeRows, kLifeGosperGliderGun,
                                   sizeof(kLifeGosperGliderGun) / sizeof(kLifeGosperGliderGun[0]), 18, 18, 36, 9);
    clearAndStampPackedLifePattern(standbyLifeCells_, kStandbyLifeColumns, kStandbyLifeRows, kLifeGosperGliderGun,
                                   sizeof(kLifeGosperGliderGun) / sizeof(kLifeGosperGliderGun[0]),
                                   static_cast<int>(kStandbyLifeColumns) - 62, static_cast<int>(kStandbyLifeRows) - 34,
                                   36, 9);
    clearAndStampPackedLifePattern(standbyLifeCells_, kStandbyLifeColumns, kStandbyLifeRows, kLifePulsar,
                                   sizeof(kLifePulsar) / sizeof(kLifePulsar[0]),
                                   static_cast<int>(kStandbyLifeColumns / 2) - 7,
                                   static_cast<int>(kStandbyLifeRows / 2) - 7, 13, 13);
    clearAndStampPackedLifePattern(standbyLifeCells_, kStandbyLifeColumns, kStandbyLifeRows, kLifePentadecathlon,
                                   sizeof(kLifePentadecathlon) / sizeof(kLifePentadecathlon[0]),
                                   static_cast<int>(kStandbyLifeColumns / 3), static_cast<int>(kStandbyLifeRows) - 42,
                                   5, 10);
    clearAndStampPackedLifePattern(standbyLifeCells_, kStandbyLifeColumns, kStandbyLifeRows, kLifeLightweightSpaceship,
                                   sizeof(kLifeLightweightSpaceship) / sizeof(kLifeLightweightSpaceship[0]),
                                   static_cast<int>((kStandbyLifeColumns * 2) / 3),
                                   static_cast<int>(kStandbyLifeRows / 3), 5, 4);

    for (uint8_t i = 0; i < 10; ++i) {
        const int x = static_cast<int>((advanceStandbyRng(standbyScreensaverRng_) >> 8)
                                       % std::max<uint16_t>(1, kStandbyLifeColumns - 6));
        const int y = static_cast<int>((advanceStandbyRng(standbyScreensaverRng_) >> 8)
                                       % std::max<uint16_t>(1, kStandbyLifeRows - 6));
        clearAndStampPackedLifePattern(standbyLifeCells_, kStandbyLifeColumns, kStandbyLifeRows, kLifeGlider,
                                       sizeof(kLifeGlider) / sizeof(kLifeGlider[0]), x, y, 3, 3);
    }
}
        ...
        for (uint16_t x = 0; x < kStandbyLifeColumns; ++x) {
            ...
            for (int8_t dy = -1; dy <= 1; ++dy) {
                for (int8_t dx = -1; dx <= 1; ++dx) {
                    ...
                    const uint16_t nx =
                        static_cast<uint16_t>((static_cast<int>(x) + dx + kStandbyLifeColumns) % kStandbyLifeColumns);
                    const uint16_t ny =
                        static_cast<uint16_t>((static_cast<int>(y) + dy + kStandbyLifeRows) % kStandbyLifeRows);
                    neighbours +=
                        packedLifeCellAlive(standbyLifeCells_, static_cast<size_t>(ny) * kStandbyLifeColumns + nx) ? 1
                                                                                                                   : 0;
                }
            }
...
void App::seedStandbyMaze(uint32_t nowMs) {
    ...
    standbyMazeStack_.clear();
    standbyLifeGeneration_ = 0;
    standbyScreensaverRng_ = nowMs ^ micros() ^ (static_cast<uint32_t>(reader_.currentIndex() + 1) * 2246822519UL);

    const uint16_t startX = static_cast<uint16_t>((advanceStandbyRng(standbyScreensaverRng_) >> 8) % mazeColumns);
    const uint16_t startY = static_cast<uint16_t>((advanceStandbyRng(standbyScreensaverRng_) >> 8) % mazeRows);
    ...
    setPackedLifeCellAt(standbyLifeCells_, kStandbyLifeColumns, kStandbyLifeRows, static_cast<int>(startX) * 2 + 1,
...
void App::stepStandbyMaze() {
    ...
    for (uint8_t step = 0; step < kMazeStepsPerFrame && !standbyMazeStack_.empty(); ++step) {
        ...
        auto addCandidate = [&](int nx, int ny) {
            if (nx < 0 || ny < 0 || nx >= static_cast<int>(mazeColumns) || ny >= static_cast<int>(mazeRows)) {
                return;
            }
        ...
        const uint16_t next = candidates[(advanceStandbyRng(standbyScreensaverRng_) >> 16) % candidateCount];
        ...
        setPackedLifeCellAt(standbyLifeCells_, kStandbyLifeColumns, kStandbyLifeRows, displayNx, displayNy, true);
        setPackedLifeCellAt(standbyLifeCells_, kStandbyLifeColumns, kStandbyLifeRows, (displayCx + displayNx) / 2,
                            (displayCy + displayNy) / 2, true);
    }
...
void App::seedStandbyVoronoi(uint32_t nowMs) {
    ...
    standbyScreensaverRng_ =
        nowMs ^ micros() ^ (static_cast<uint32_t>(reader_.currentIndex() + 1) * 3266489917UL) ^ 0x51a7f00dUL;
    ...
    for (size_t i = 0; i < kVoronoiSiteCount; ++i) {
        standbyVoronoiX_[i] =
            static_cast<int16_t>(((advanceStandbyRng(standbyScreensaverRng_) >> 8) % kStandbyLifeColumns) * 16);
        standbyVoronoiY_[i] =
            static_cast<int16_t>(((advanceStandbyRng(standbyScreensaverRng_) >> 8) % kStandbyLifeRows) * 16);

        const int16_t dx = static_cast<int16_t>(4 + ((advanceStandbyRng(standbyScreensaverRng_) >> 24) % 7));
        const int16_t dy = static_cast<int16_t>(3 + ((advanceStandbyRng(standbyScreensaverRng_) >> 24) % 6));
        standbyVoronoiDx_[i] = (advanceStandbyRng(standbyScreensaverRng_) & 1U) != 0 ? dx : static_cast<int16_t>(-dx);
        standbyVoronoiDy_[i] = (advanceStandbyRng(standbyScreensaverRng_) & 1U) != 0 ? dy : static_cast<int16_t>(-dy);
    }
...
void App::stepStandbyVoronoi() {
    constexpr size_t kVoronoiSiteCount = 15;
    if (standbyVoronoiX_.size() != kVoronoiSiteCount || standbyVoronoiY_.size() != kVoronoiSiteCount
        || standbyVoronoiDx_.size() != kVoronoiSiteCount || standbyVoronoiDy_.size() != kVoronoiSiteCount) {
...
void App::updateStandbyScreensaver(uint32_t nowMs, bool force) {
    ...
    lastStandbyFrameMs_ = nowMs;
    display_.renderLifeScreensaver(standbyLifeCells_, kStandbyLifeColumns, kStandbyLifeRows, standbyLifeGeneration_,
                                   standbyScreensaverDimCells_.empty() ? nullptr : &standbyScreensaverDimCells_);
}
...
void App::enterPowerOff(uint32_t nowMs) {
    ...
    const uint32_t waitStartMs = millis();
    while ((readLogicalPowerButtonHeld() || readLogicalPrimaryButtonHeld())
           && millis() - waitStartMs < kPowerOffReleaseWaitMs) {
        delay(10);
    }
...
void App::wakeFromSleep() {
    ...
    if (storageReady_ && usingStorageBook_ && !currentBookPath_.isEmpty()) {
        ...
        reloadOptions.allowEpubConversion = false;
        reloadOptions.rebuildTimeEstimate = false;
        if (refreshedBookIndex >= 0 && loadBookAtIndex(static_cast<size_t>(refreshedBookIndex), nowMs, reloadOptions)) {
            reader_.seekTo(resumeIndex);
        } else {
...
bool App::loadBookAtIndex(size_t index, uint32_t nowMs, const BookOpenOptions& options) {
    ...
    const bool keepingExistingTimeCache =
        !options.rebuildTimeEstimate && timeEstimateCacheValid_ && currentBookPath_ == loadedPath;
    ...
    Serial.printf("[app] loaded SD book[%u/%u]: %s (%u chapters, %u paragraphs)\n",
                  static_cast<unsigned int>(loadedIndex + 1), static_cast<unsigned int>(storage_.bookCount()),
                  loadedPath.c_str(), static_cast<unsigned int>(chapterMarkers_.size()),
...
void App::renderTypographyTuning() {
    ...
    const size_t beforeIndex = index == 0 ? kTypographyPreviewWordCount - 1 : index - 1;
    const size_t afterIndex = (index + 1 >= kTypographyPreviewWordCount) ? 0 : index + 1;
    ...
    const String title = uiText(UiText::Typography) + " " + String(static_cast<unsigned int>(index + 1)) + "/"
    ...
    display_.renderTypographyPreview(beforeText, kTypographyPreviewWords[index], afterText, readerFontSizeIndex_, title,
                                     line1, line2);
}
...
void App::renderFocusTimerSession() {
    ...
    case FocusTimer::State::WorkRunning:
        display_.renderFocusTimerScreen("WORK", "", remainingLabel, "", "", focusTimer_.progressPercent(millis()));
        return;
    case FocusTimer::State::BreakRunning:
        display_.renderFocusTimerScreen("BREAK", "", remainingLabel, "", "", focusTimer_.progressPercent(millis()),
                                        true);
        return;
    case FocusTimer::State::WaitAfterWork:
        display_.renderFocusTimerScreen("BREAK", "", "", "Turn for break", "", -1, true);
        return;
...
bool App::maybeStartChapterTransition(size_t previousWordIndex, size_t currentWordIndex, uint32_t nowMs) {
...
String App::chapterMenuLabel(size_t chapterIndex) const {
    ...
    const size_t endIndex =
        (chapterIndex + 1 < chapterMarkers_.size()) ? chapterMarkers_[chapterIndex + 1].wordIndex : reader_.wordCount();
...
String App::currentFooterMetricLabel() const {
    ...
    const bool generatingEstimate =
        accurateTimeEstimateEnabled_ && timeEstimateBuildInProgress_ && timeEstimateBuildMatchesCurrentBook();
    const int generatingPercent = generatingEstimate
    ...
    if (footerMetricMode_ == FooterMetricMode::ChapterTime) {
        ...
        return String("CH ") + formatReadingTimeRemaining(estimatedReadingTimeRemainingMs(currentIndex, endIndex));
    }

    if (generatingEstimate) {
        return String("BOOK ") + String(generatingPercent) + "% gen";
    }
    return String("BOOK ") + formatReadingTimeRemaining(estimatedReadingTimeRemainingMs(currentIndex, endIndex));
}
...
String App::batteryVoltageLabel() const {
    return String(batteryFilteredVoltage_, 2) + "V";
}
...
uint32_t App::estimatedReadingTimeRemainingMs(size_t startIndex, size_t endIndex) const {
    ...
    const uint32_t baseMs = static_cast<uint32_t>((static_cast<uint64_t>(endIndex - startIndex) * 60000ULL)
...
uint32_t App::estimatedPacingBonusMs(size_t startIndex, size_t endIndex) const {
    if (!timeEstimateCacheValid_ || wordBonusBlockPrefixSumMs_.empty() || endIndex <= startIndex) {
        return 0;
    }
    ...
    const size_t firstFullBlock = (startIndex + kTimeEstimateBlockWords - 1) / kTimeEstimateBlockWords;
    const size_t lastFullBlockEnd = endIndex / kTimeEstimateBlockWords;
    uint32_t bonusMs = 0;

    if (firstFullBlock < lastFullBlockEnd && lastFullBlockEnd < wordBonusBlockPrefixSumMs_.size()) {
        const size_t startPartialEnd = std::min(endIndex, firstFullBlock * kTimeEstimateBlockWords);
        for (size_t i = startIndex; i < startPartialEnd; ++i) {
            bonusMs += reader_.wordPacingBonusMsAt(i);
        }

        bonusMs += wordBonusBlockPrefixSumMs_[lastFullBlockEnd] - wordBonusBlockPrefixSumMs_[firstFullBlock];
...
void App::rebuildTimeEstimateCache() {
    ...
    if (!accurateTimeEstimateEnabled_) {
        if (!currentBookTitle_.isEmpty()) {
            renderStorageStatus("Reading time", currentBookTitle_.c_str(), "Fast estimate enabled", 100);
        }
        return;
    }
    ...
    Serial.printf("[time-est] background build started words=%u blocks=%u book=%s\n",
                  ...
                  static_cast<unsigned int>(timeEstimateBuildBlockCount_), currentBookPath_.c_str());
}
...
void App::updateTimeEstimateBuild(uint32_t nowMs) {
    ...
    if (state_ == AppState::Playing || state_ == AppState::CompanionSync || state_ == AppState::UsbTransfer
        || state_ == AppState::Standby || state_ == AppState::Sleeping) {
        return;
    }
    ...
    while (timeEstimateBuildNextBlock_ < timeEstimateBuildBlockCount_
        ...
        const size_t blockEnd = std::min(timeEstimateBuildWordCount_, blockStart + kTimeEstimateBlockWords);
    ...
    if (timeEstimateBuildNextBlock_ >= timeEstimateBuildBlockCount_) {
        ...
        Serial.printf("[time-est] background cached %u words in %u blocks bonus=%lums took=%lums\n",
                      ...
                      static_cast<unsigned long>(timeEstimateBuildRunningMs_), static_cast<unsigned long>(elapsedMs));
        cancelTimeEstimateBuild();
        if (state_ == AppState::Paused || state_ == AppState::Playing) {
            renderActiveReader(nowMs);
        } else if (state_ == AppState::Menu) {
    ...
    if (nowMs - timeEstimateBuildLastLogMs_ >= kTimeEstimateProgressLogMs) {
        const int progress =
            static_cast<int>((timeEstimateBuildNextBlock_ * 100UL) / std::max<size_t>(1, timeEstimateBuildBlockCount_));
...
String App::timeEstimateModeLabel() const {
    return uiText(accurateTimeEstimateEnabled_ ? UiText::TimeEstimateAccurate : UiText::TimeEstimateFast);
}
...
Board::UiOrientation App::readerUiOrientation() const {
    return handednessMode_ == HandednessMode::Right ? Board::Display::defaultUiOrientation()
...
String App::focusTimerCountsLabel() const {
    return "T" + String(focusTimer_.completedTouchBlocks()) + " W" + String(focusTimer_.completedWorkBlocks()) + " B"
...
bool App::scrollModeEnabled() const {
    return readerMode_ == ReaderMode::Scroll;
}

bool App::uiRotated180() const {
    const Board::UiOrientation orientation = readerUiOrientation();
    return orientation == Board::UiOrientation::LandscapeFlipped ||
...
void App::handleCurrentBookReadFailure(uint32_t nowMs, const char* detail) {
    ...
    const bool articlesOnly = currentBookIndex_ < storage_.bookCount() && storage_.bookIsArticle(currentBookIndex_);
    ...
    setState(AppState::Menu, nowMs);
    display_.renderStatus("Book read failed", failedTitle, detail == nullptr ? "Reopen from library" : detail);
    delay(1800);
...
void App::renderReaderWord() {
    ...
    display_.renderPhantomRsvpWord(beforeText, reader_.currentWord(), afterText, readerFontSizeIndex_,
                                   currentChapterLabel(), readingProgressPercent(), showReaderFooter, footerMetricLabel,
                                   chrome);
}
...
void App::renderScrollReader(uint32_t nowMs, const String& overlayText) {
    ...
    if (state_ == AppState::Playing && currentIndex + 1 < wordCount) {
        ...
        if (durationMs > 0) {
            ...
            scrollProgressPermille =
                static_cast<uint16_t>(std::min<uint32_t>(1000UL, (elapsedMs * 1000UL) / durationMs));
        }
    }
...
void App::renderWpmFeedback(uint32_t nowMs) {
    ...
    const DisplayManager::ReaderChrome chrome = readerChrome();
    const String footerMetricLabel = readerFooterStatusLabel();
    display_.renderPhantomRsvpWordWithWpm(beforeText, reader_.currentWord(), afterText, readerFontSizeIndex_,
                                          reader_.wpm(), currentChapterLabel(), readingProgressPercent(),
...
void App::handleStorageStatus(void* context, const char* title, const char* line1, const char* line2,
                              int progressPercent) {
    if (context == nullptr) {
        return;
    }
