...
class DisplayManager {
 public:
  ...
  // glyphs/brightness/lead are row-major (index = row * columns + column); brightness 0 means
  // "no glyph at this cell", and non-zero blends baseColor over the background. lead[index]
  // marks the trail's brightest (head) cell, drawn in solid white when highlightsEnabled.
  // frameCounter must change every call so the dirty-frame cache never suppresses an
  // animation frame.
  void renderDigitalRain(const std::vector<char> &glyphs, const std::vector<uint8_t> &brightness,
                         const std::vector<bool> &lead, uint16_t columns, uint16_t rows,
                         uint16_t cellWidth, uint16_t cellHeight, uint16_t baseColor,
                         uint8_t glyphScale, bool highlightsEnabled, uint32_t frameCounter);
